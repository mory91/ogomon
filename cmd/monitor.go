...
func (m Monitor) Start(appendFile bool) error {
	...
	sendMsgFdCommand := exec.Command("sudo", "./python/sendmsg_fd.py", "-p", fmt.Sprintf("%d", stat.PID))
	sendToFdCommand := exec.Command("sudo", "./python/sendto_fd.py", "-p", fmt.Sprintf("%d", stat.PID))
